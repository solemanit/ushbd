                        ...
                        <div class="mb-3 shadow-sm card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3 text-center">
                                    ...
                                    <img id="imagePreview"
                                        src="{{ $product->image ? asset('storage/' . $product->image) : asset('images/placeholder-image.webp') }}"
...
@push('scripts')
    <script>
        ...
        const populateSelect = (select, data, placeholder = 'Select option') => {
            ...
            data.forEach(item => {
                select.innerHTML += `<option value="${item.id}">${item.name} &nbsp;–&nbsp; ${item.bn_name}</option>`;
            });
        ...
        async function loadDivisions() {
            try {
                ...
                @if($product->division_id)
                    divisionSelect.value = '{{ $product->division_id }}';
                    loadDistricts('{{ $product->division_id }}');
                @endif
            } catch (err) {
        ...
        async function loadDistricts(divisionId) {
            try {
                ...
                if (data.success) {
                    populateSelect(districtSelect, data.data, 'Select District');
                    @if($product->district_id)
                        districtSelect.value = '{{ $product->district_id }}';
                    @endif
                }
            } catch (err) {
